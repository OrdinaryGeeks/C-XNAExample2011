...
namespace SmellOfRevenge2011
{
    ...
    public class InputState
    {
        ...
        public void UpdatePause(bool script)
        {
            for (int i = 0; i < MaxInputs; i++)
            {
                ...
                if (script)
                {
                    if (CurrentGamePadStates[i].IsButtonDown(Buttons.A) && LastGamePadStates[i].IsButtonUp(Buttons.A))
                    {
                        ScreenManager.script = false;
                        pause = false;
                    }
                ...
                if (CurrentGamePadStates[i].IsConnected)
                {
                    ...
                    if (CurrentGamePadStates[i].IsButtonDown(Buttons.Start) && LastGamePadStates[i].IsButtonUp(Buttons.Start))
                        if (pause)
                            pause = false;
                        else
                            pause = true;
                ...
                else if (GamePadWasConnected[i]) //if it was connected and isnt nowpause the game
                    pause = true;
